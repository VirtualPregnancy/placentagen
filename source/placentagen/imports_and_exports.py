...
def import_stem_location(node,stemstart,stemend):

   stemnode=np.arange(stemstart-1,stemend)
   stem_coor=np.zeros((len(stemnode),2))

   for i in range (0,len(stemnode)):

     stem_coor[i,0]=node[stemnode[i],1]#x coor of stem villi
     stem_coor[i,1]=node[stemnode[i],2]#y coor of stem villi
